...
describe("ImgwireClient", () => {
  ...
  it("extends created images with url generation", async () => {
    ...
    expect(
      ...
      "https://cdn.imgwire.dev/example@thumbnail?height=150&rotate=90&width=150"
    );
  });
  ...
  it("fetches images by id and returns an extended image object", async () => {
    const fetchMock = vi.fn<typeof fetch>().mockResolvedValueOnce(
      new Response(
        JSON.stringify({
          ...createMockImage({
            cdn_url: "https://cdn.imgwire.dev/uploaded",
            height: 400,
            id: "img_fetch_123",
    ...
    ).toBe("https://cdn.imgwire.dev/uploaded?rotate=90");
  });
